...
public class RSVD2BPR {
	...
	// === users in initial train data(rating > 0)
	...
	// === candidate list
	public static HashMap<Integer, ArrayList<Integer>> CandidateList = new HashMap<Integer, ArrayList<Integer>>();
	...
	public static void main(String[] args) throws Exception {
		...
		for (int k = 0; k < args.length; k++) {
			if (args[k].equals("-d"))
				...
				flagInputCandidateList = true;
				fnInputCandidateList = args[++k];
		...
		System.out.println("fnInputCandidateList: " + fnInputCandidateList);
		...
		// =========================================================
		// === Step 2: Read candidatelist
		if (fnInputCandidateList.length() > 0) {
			long TIME_START_ReadCandidateList = System.currentTimeMillis();
			ReadCandidateList.readCandidateList();
			long TIME_FINISH_ReadCandidateList = System.currentTimeMillis();
			System.out.println("Elapsed Time ( read CandidateList):"
					+ Float.toString((TIME_FINISH_ReadCandidateList - TIME_START_ReadCandidateList) / 1000F) + "s");
		}
		// =========================================================

		// =========================================================
		// === Step 3: Initialization of U, V, bias
		...
		System.out.println("Elapsed Time (init):"
				+ Float.toString((TIME_FINISH_INITIALIZATION - TIME_START_INITIALIZATION) / 1000F) + "s");
		// =========================================================
		...
		System.out.println(
				"Elapsed Time (training):" + Float.toString((TIME_FINISH_TRAIN - TIME_START_TRAIN) / 1000F) + "s");
		// =========================================================

		// =========================================================
		// === Step 5: Re-ranking and Evaluation
	...
	public static void readDataTrainTest() throws Exception {
		...
		while ((line = br.readLine()) != null) {
			...
			// --- add to the initial Train user set
			userSetTrain.add(userID);

			// --- select rating=5 as training data
			if (rating < 5) { 
				continue;
			}
			
			// --- add to the traning item set(=5)
			...
			// --- the number of user-item pairs in train data
			num_train += 1; 
		...
		br.close();
		System.out.println("The number of users in the test data: " + TestData.size());
		// =========================================================
	}
	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	public static void testRanking(HashMap<Integer, HashSet<Integer>> TestData) throws IOException {
		...
		for (int u = 1; u <= n; u++) {
			...
			// --- check whether the user $u$ is in the initial train set
			...
			if (TrainData.containsKey(u)) { // TrainData(rating=5)
			...
			} else { // user in CandidateList but not in Train data
				ArrayList<Integer> ItemSet_u_TopKItem = CandidateList.get(u);
				for (int i : ItemSet_u_TopKItem) {
					if (k > topK)
						break;
			...
			for (k = 1; k <= topK; k++) {
				...
				RecallSum[k] += rec;
				F1Sum[k] += F1;
				// --- in case the the number relevant items is smallerthan k
