...
public class RSVD_15
{
    ...
    public static HashSet<Integer> ItemSetTrain = new HashSet<Integer>(); // ==whole data(items)
    ...
    {	
    	...
    	// --- some statistics, start from index "1"
        userRatingSumTrain = new float[n+1];
	...
	public static void testRanking(HashMap<Integer, HashSet<Integer>> TestDataforRank) throws IOException
    {
    	...
    	for(int u=1; u<=n; u++)
		{
			...
			if (!TestDataforRank.containsKey(u) || !TrainData.containsKey(u)) { // warm-start
				continue;
			}
			...
			for (int i=1; i< m+1; i++) {
				...
				float pred = g_avg + biasU[u] + biasV[i];
				for (int f = 0; f < d; f++) {
					pred += U[u][f] * V[i][f];
				}
